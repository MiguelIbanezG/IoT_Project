...
def process_pose(frame):
    ...
    for person in keypoints_with_scores:
        ...
        draw_connections(frame, person, EDGES, 0.25)
        draw_keypoints(frame, person, 0.25)
...
def fetch_frame(token):
    url = "https://danubio.ii.uam.es/api/camera_proxy/camera.amicam"
    headers = {
    ...
    if response.status_code == 200:
        np_arr = np.asarray(bytearray(response.content), dtype=np.uint8)
...
# Función para capturar frames en tiempo real desde una cámara (reemplaza la URL)
def capture_frames(token, fps=10):
    cap_time = 1 / fps
    prev_time = time.time()

    while True:
        # Aquí debes reemplazar `fetch_frame` con la función que captura frames de la cámara
        frame = fetch_frame(token)
        ...
        # start_det = time.time()
        # detections = detector.detectObjectsFromImage(input_image=frame, minimum_percentage_probability=35)
        # end_det = time.time()
        # print(f"Tiempo de detección de objetos: {end_det - start_det:.4f} segundos")
        ...
        # Mostrar el video con las detecciones y poses
        cv2.imshow("Video en tiempo real", frame)
        print(f"Total de personas detectadas: {person_count}")

        # Calcular el tiempo de espera para mantener los 10 fps
        elapsed_time = time.time() - prev_time
        wait_time = max(cap_time, cap_time - elapsed_time)
        print(f"Tiempo de espera: {wait_time:.4f} segundos")
        prev_time = time.time()
...
access_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI2ODcxZmRkNzgyODE0NzUzOTI3NzNlYmZlN2FkMzkxOCIsImlhdCI6MTc0MjMxMDU5NiwiZXhwIjoxNzQyMzEyMzk2fQ.jJjyJOSm3kRVvPEIhkHcaOsJZCTLkogVIQLp6uGS47U"

# Procesar video y detectar objetos y poses a 10 fps
capture_frames(access_token, fps=15)